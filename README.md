...
[Поставка](./Projektni_zadatak_PISIBP_2025.pdf).
...
## Ток извршавања функционалности

### Регистрација и пријава

1. Корисник отвара апликацију (`http://localhost:5173`) и бива преусмерен на `/login` страницу
2. Нови корисник кликне на "Региструј се", попуни формулар (име, презиме, корисничко име, **email**, лозинка) и потврди
3. `auth-service` креира налог у својој бази и позива `user-service` (интерно) да креира профил
4. Након успешне регистрације, корисник се аутоматски пријављује - `auth-service` издаје **JWT** токен који се чува у `localStorage`
5. Сваки наредни захтев ка било ком сервису садржи `Authorization: Bearer <token>` header

### Временска линија (Feed)

1. Након пријаве, корисник види временску линију - хронолошки сортиране објаве профила које прати
2. `feed-service` преузима листу праћених профила од `follow-service`, филтрира блокиране кроз `blok-service` и узима објаве из `post-service`
3. Корисник може скроловати кроз објаве, лајковати их и коментарисати директно из feed-a

### Претрага и преглед профила

1. Корисник кликне на иконицу претраге у navbar-у и унесе текст
2. `user-service` претражује кориснике по имену или корисничком имену, филтрирајући блокиране кроз `blok-service`
3. Кликом на резултат отвара се профил корисника где се виде основне информације, број пратилаца/праћених и галерија објава
4. За приватне профиле, галерија и листе пратилаца су скривене док корисник не буде прихваћен као пратилац

### Праћење профила

1. На профилу другог корисника, корисник кликне на "Запрати"
2. Ако је профил **јавни** - `follow-service` аутоматски креира релацију праћења и шаље обавештење
3. Ако је профил **приватни** - `follow-service` креира захтев за праћење. Власник профила добија обавештење и може да прихвати или одбије захтев
4. Када корисник пребаци профил са приватног на јавни, сви **pending** захтеви се аутоматски прихватају

### Објављивање садржаја

1. Корисник кликне на иконицу "+" у navbar-у, отвара се **CreatePost**
2. Одабере слике/видео клипове (максимално 20, до 50 MB по фајлу), додаје опис и кликне "Објави"
3. `post-service` валидира фајлове, скалира слике (макс. 1080x1080) и чува их на диску
4. Објава се појављује на профилу корисника и у feed-у свих пратилацаа

### Интеракција са објавама

1. Корисник може да **лајкује** објаву кликом на срце - `interactive-service` чува лајк и шаље обавештење власнику
2. Корисник може да **коментарише** објаву - коментар се приказује испод објаве, уз обавештење које се шаље власнику
3. Своје коментаре корисник може да измени или обрише
4. Власник објаве може обрисати било чији коментар на својој објави
5. Лајкови и коментари блокираних корисника се не рачунају и не приказују

### Блокирање

1. На профилу другог корисника, корисник кликне на мени (три тачке) и одабере опцију за блокирање
2. `blok-service` креира блокаду и позива `follow-service` да уклони евентуално постојећу релацију праћења у оба смера
3. Блокирани корисник не може пронаћи профил у претрази, видети објаве, лајковати или коментарисати

### Уређивање профила

1. Корисник на сопственом профилу кликне "Измени профил" и преусмерен је на `/edit-profile`
2. Корисник може променити име, презиме, корисничко име, биографију, профилну слику и приватност
3. Промена корисничког имена се синхронизује са `auth-service` (интерни позив)

### Брисање налога

1. Корисник у подешавањима одабере брисање налога
2. `auth-service` каскадно позива све сервисе (интерним ендпоинтима) да обришу податке корисника: објаве, лајкове, коментаре, **follow** релације, блокове и профил
3. На крају, `auth-service` брише налог из своје базе
